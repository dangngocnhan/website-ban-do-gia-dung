            ...
            <div class="table-content">
                <table>
                 ...
                 if($show_payment){$i=0;while($result = $show_payment->fetch_assoc()){$i++;
                    $show_order = $product  -> show_order($result['order_id']);
                    if($show_order){while($result1 = $show_order->fetch_assoc()){ 
                ?>
                    <tr>
                        <td> <?php echo $i ?></td>
                        ...
                        <td><a style="color:#378000" href="?status=1&session_idA=<?php echo $result['session_idA'].'&payment_id='.$result['payment_id'].'&order_id='.$result['order_id'] ?>">Đã hoàn thành</a></td>
